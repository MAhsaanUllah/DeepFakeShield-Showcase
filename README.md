# 🛡️ DeepFakeShield: Enterprise-Grade Deepfake & Synthetic Media Detection System
...
```mermaid
graph TD
    Client[React/Vite Frontend] -->|HTTPS POST Payload| Gateway[FastAPI Azure Backend]
    Gateway --> RateLimit{Rate Limit Check}
    RateLimit -- Allow --> Media[Media Processing / OpenCV Haar]
    Media --> CNN[PyTorch CNN Ensemble]
    CNN --> Xcep[XceptionNet-41]
    CNN --> Eff[EfficientNet-B4]
    CNN --> Mob[MobileNetV3]
    CNN --> Consensus{Confidence > 85%?}
    Consensus -- Yes: Real --> Final[Return Classification & Bounding Box]
    Consensus -- No / Fake / AI --> Gemma[HuggingFace Gemma-3 VLM]
    Gemma --> XAIText[Generate XAI Forensic Justification]
    XAIText --> Final
    Final --> SQLite[(SQLite Telemetry Logger)]
```
...
* **Ensemble Consensus Algorithm:** Averaged standardized probability distributions from three parallel CNN backbones (`XceptionNet-41`, `EfficientNet-B4`, `MobileNetV3`) to yield a highly balanced, robust final prediction, eliminating single-model bias.
...
* **Telemetry & Graphs Dashboard:** A real-time analytics panel mapping live scanning traffic, model usage percentages, and classification distributions (Real vs Fake) via asynchronous SQLite database logging. Secure CSV log exporting is available for offline auditing.
